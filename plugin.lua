...
function placeAdvancedSplitScrollSVsActual(settingVars, allNoteOffsets)
    ...
    actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, 727)})})--capy
    actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, #splitscrollOffsets)})})--capy
    actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, #noteOffsets)})})--capy
    for i = 1, numScrolls do
        frameDistancesInScroll[i] = calculateDisplacementsFromSVs(svsInScroll[i], splitscrollOffsets)
        actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, 1)})})--capy
        noteDistancesInScroll[i] = calculateDisplacementsFromSVs(svsInScroll[i], noteOffsets)
        actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, 2)})})--capy
    end
    actions.PerformBatch({utils.CreateEditorAction(action_type.AddScrollVelocityBatch, {utils.CreateScrollVelocity(0, 728)})})--capy
